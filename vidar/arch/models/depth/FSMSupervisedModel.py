...
class FSMSupervisedModel(BaseModel):
    """
    ...
    def forward(self, batch, return_logs=False, progress=0.0, **kwargs):
        ...
        if self.networks['depth'] is not None:
            output_sup = self.forward2(batch)
            depth = inv2depth(output_sup['inv_depths'])
        ...
        if not self.training:
            output_new = {
                'predictions': {
                    'depth': {
                        0: [1. / d for d in output_sup['inv_depths']]
                    }
                }
            }
            output_sup = output_new
            return output_sup
        ...
        try:
            ...
            masks = [torch.tensor(np.load(f)).unsqueeze(0).unsqueeze(0) for f in filename[0]]
            for tgt in range(n_tgt):
        ...
        for tgt in range(n_tgt):
            output = self.losses['supervised'](
                rgb_i[tgt], rgb_context_i[tgt], inv_depth_i[tgt],
                cam_i[tgt], cam_context_i[tgt])
