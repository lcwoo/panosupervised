...
class PanoSupervisedModel(BaseModel):
    ...
    def rgb_lidar(self, batch, panodepth):
        ...
        for cam_name in camera_names:
            ...
            Twc = batch_cam['extrinsics'][0]
            camera = Camera(K, hw, Twc=Twc.float(), name=cam_name)
            ...
            world_points = camera_pano.reconstruct_depth_map(panodepth, to_world=True)
            coords = camera.project_points(world_points, from_world=True, normalize=True)[0]
            imag_depth, weights = self.flow_reverse(panodepth, coords, dst_img_shape=scaled_imag_shape)
    ...
    def forward(self, batch, epoch, return_logs=False):
        ...
        # pano_lidar = self.rgb_lidar(batch, gt_panodepth)
