...
class PanoCrossViewTransformer(nn.Module):
    def __init__(self,input_channel, out_size, downsample_ratio=2, iter_num=4):
        super().__init__()
        self.embed_dim = input_channel
        self.iter_num = iter_num
        self.self_block = []
        out_size_downsampled = [s // downsample_ratio for s in out_size]
        self.out_size_downsampled = out_size_downsampled
...
class Self_Block(nn.Module):
    ...
    def forward(self, x, x_pos, y_pos):
        # x = x + self.drop_path(self.attn(self.norm1(x), x_pos, y_pos))
        y = self.drop_path(self.attn(self.norm1(x), x_pos, y_pos))
...
class Self_Attention(nn.Module):
    ...
    def forward(self, x, x_pos, y_pos):
        ...
        # q = self.q_linear(q_vector).reshape(B, Ny, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        # k = self.k_linear(k_vector).reshape(B, Nx, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        # v = self.v_linear(v_vector).reshape(B, Nx, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        q = self.q_linear(q_vector).reshape(B, Ny, self.num_heads, -1).permute(0, 2, 1, 3)
        k = self.k_linear(k_vector).reshape(B, Nx, self.num_heads, -1).permute(0, 2, 1, 3)
        v = self.v_linear(v_vector).reshape(B, Nx, self.num_heads, -1).permute(0, 2, 1, 3)
...
class PositionEmbeddingSine(nn.Module):
    ...
    def forward(self, x):
        ...
        # dim_t = self.temperature ** (2 * (dim_t // 2) / float(self.num_pos_feats))
        # dim_t = self.temperature ** torch.div(2 * (dim_t // 2), self.num_pos_feats, rounding_mode='floor')
        dim_t = self.temperature ** torch.div(2 * torch.div(dim_t, 2), self.num_pos_feats, rounding_mode='floor')
        # pdb.set_trace()
        pos_x = x_embed[:, :, :, None] / dim_t
        pos_y = y_embed[:, :, :, None] / dim_t
        pos_x = torch.stack((pos_x[:, :, :, 0::2].sin(), pos_x[:, :, :, 1::2].cos()), dim=4).flatten(3)
